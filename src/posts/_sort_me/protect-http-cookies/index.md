...
This new behavior is still [not completely supported by all the browsers](https://caniuse.com/#search=samesite) as of 11/2023. Chrome supports this, starting with version 80, the same goes with Edge. In Firefox, you can explicitly enable the support since version 69, however, it is not enabled by default. You can still use `Samesite=None` as it is backward compatible and will be ignored by older browsers.
