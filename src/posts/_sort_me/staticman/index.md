...
Note the `/comments` section at the end of action URL. This is a section identifier (marked as `{PROPERTY}` in the url template above), which allows you to have multiple configurations for your site. E.g. One for comments, one for user reviews etc. In this case, we're saying that we want to use configuration specified under the `comments` section of your configuration file.
