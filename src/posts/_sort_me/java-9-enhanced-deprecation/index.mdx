...
excerpt: '@Deprecated annotation, introduced in Java 5, received several enhancements in Java 9, which makes easier to document deprecated API.'
draftStatus: draft
---
...
In the JDK from version 9 on, there is included a special tool for analyzing code for usage of deprecated API called *jdeprscan*, which can be found under `JDK_HOME\bin`. Currently, it supports scanning of JDK itself, but an extension is planned to support also external classes. For dynamic scanning of deprecated APIs, an introduction of a special tool *jdeprdetect* is considered, but it will not be part of Java 9.

More info about the *jdeprscan* tool can be found [here](https://docs.oracle.com/javase/9/tools/jdeprscan.htm#JSWOR-GUID-2B7588B0-92DB-4A88-88D4-24D183660A62).

## IDE Support

Current IDEs already offer nice support for warning you when you are using some deprecated code. IntelliJ IDEA, for example, will strike through any usage of deprecated code. With java 9+ and the enhanced deprecation, it will even show you a stronger warning when the deprecated code is issued for removal - the strikethrough is red in such case.
