...
<Video src={props.data.mdx.frontmatter.path+"idea-surround-with-try-with-resources"} />

Using the same keyboard shortcut, IDEA allows you to convert traditional `try-catch-finally` to `try-with-resources`.

<Video src={props.data.mdx.frontmatter.path+"idea-replace-with-try-with-resources"} />
