...
Alternatively, if you don't mind doing some extra works, custom search offer an [API](https://developers.google.com/custom-search/json-api/v1/overview), so you can retreive results programatically and seamlessly integrate the results in your page. This, however, somewhat negates the greates advantage of Google Custom search, which is its easy setup.
...
There are some advantages over Google Search though. First of all, no ads. And everything integrates with look and feel of your site nicely as you just mark a text input on your page as a search box to be handled by DocSearch. It then attaches autocomplete functionality to this search box. Whenever you start typing you get results from DocSearch. Directly in your page with possibility to easily override default styles. And setup is really easy and your pages are indexed for you automatically.

![Algolia DocSearch](./docsearch.png)

### Requesting access
The first step you need to make is request DocSearch for your site. Just fill a simple form on [their website](https://community.algolia.com/docsearch/).
...
However, after the initial rejection I did ask nicely whether they can provide the service for my blog. They were really kind to me and registered me anyway. 

> DocSearch is focused on documentation content that helps the community. It looks like your content is helpful this why we will provide you DocSearch.
...
One of the advantages of DocSearch how easy it is to set up and configure. You just need to add the following to your page (Algolia team will provide you with exact values and details):
...
You just need to provide proper selector (such as css class or id) for the input search element. Algolia team will also configure the project for you and store the configuration file in their git repo. In case you need to make any changes, you just create a Pull Request, which gets quickly merged. In the config file, you define hierarchy of your pages sections. That means for example that H1 headings will have more weight in search results than H2, H3 will have even less weight and ordinary paragraphs will have the least relevance. With custom selectors you can define where exactly in your page is your content, so you can easily ignore sections such as footer or navigation. Here is an example of a simple configuratio for my site:
...
If you're looking for more complex example, you can check [configuration of GatsbyJs](https://github.com/algolia/docsearch-configs/blob/master/configs/gatsbyjs.json).

## Algolia Search
DocSearch is primarily focused on documentation and full text search. However, in your case you may need something more powerful and sophisticated. In such case, Algolia's flagship product - Algolia Search is better for you.

Instead of working with full text of your pages, you can define your own structured data in form of JSON objects and Algolia works with them. Each of these objects can have many properties and you can search across all of them.

On top of this Algolia offers many advanced features, which are not available in DocSearch. You can define how the search results should be ranked so the user recevies the most relevant results. You can define synonyms for search results. You can provide correction alternatives to common typos. There is a nice dashboard and search logs. You can even have analytics with a paid plan.

### Uploading search data
Algolia does not crawl your content automatically as DocSearch does. You need to upload data to their servers using [API](https://www.algolia.com/doc/api-reference/). They offer clients for number of languages and frameworks such as Java, C#, PHP, Python, Django, Laravel, Rails or Symfony.
...
Of course you can do it manually and it will work. With some static site generators it is your only option. Fortunately some of the generators provide plugins for Algolia, which will make the process much easier. For example there is a plugin for [Jekyll](https://github.com/algolia/jekyll-algolia) or [GatsbyJs](https://github.com/algolia/gatsby-plugin-algolia) (as of 6/2018 in beta). 

## Search your videos
If you also work with videos, Algolia offers another very interesting service. It is called [Algolia TalkSeach](https://community.algolia.com/talksearch/). It allow you to add search to your youtube videos. It fetches transcript for the video and makes it searchable, so you can easily navigate to the part of the video you are interested in. 
...
Having client-side search offers a fast solution, which can be good if you want your app to work offline as a Progressive Web App. Of course, you need to download the whole search index to the client, which can bee too much once you have a lot of content.

Algolia DocSearch is an easy to set up alternative, which will crawl your site for you. And the data is stored on the server so you don't have to download everything.

If you need more robust approach and more control, regular Algolia may be better solution, but the setup is somewhat more complex as you need to upload your data when you make changes to your site.