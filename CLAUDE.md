...
output filenames contain a content hash, so a stale cache is always safe. Cache lives in `.image-cache/` (gitignored;
persisted via actions/cache in CI) or `node_modules/.cache/image-mirror` on Cloudflare Pages. CI builds a lean variant
via `ELEVENTY_IMAGE_FORMATS`/`ELEVENTY_IMAGE_WIDTHS` env vars. Details in docs/IMAGES.md.
