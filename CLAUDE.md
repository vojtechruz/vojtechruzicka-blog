...
Posts are excluded from production builds by default. Control via frontmatter `draft: true` and the `INCLUDE_DRAFTS`
environment variable (`all` or `none`). Draft posts live in `src/posts/_drafts/`.
...
The site has no service worker of its own. `src/static/sw.js` is a temporary self-destroying worker that unregisters
the old Gatsby-era one from visitors' browsers — do not treat it as an entry point for offline support. Details and
removal date in docs/SERVICE-WORKER.md.
...
(gitignored) everywhere: persisted via actions/cache in CI and by the Cloudflare Pages build cache (the Eleventy
framework preset preserves `.cache`). CI builds a lean variant via `ELEVENTY_IMAGE_FORMATS`/`ELEVENTY_IMAGE_WIDTHS` env
vars. Details in docs/IMAGES.md.
